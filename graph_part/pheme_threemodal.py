...
class Signed_GAT(nn.Module):
    def __init__(self,node_embedding,cosmatrix,nfeat, uV, original_adj, hidden = 16, \
        ...
        self.original_adj = torch.from_numpy(original_adj.astype(np.float64)).to(device)
        self.potentinal_adj = torch.where(cosmatrix>0.5,torch.ones_like(cosmatrix),torch.zeros_like(cosmatrix)).to(device)
    ...
    def forward(self, X_tid):
        X = self.user_tweet_embedding(torch.arange(0, self.uV).long().to(device)).to(torch.float32)
...
class NeuralNetwork(nn.Module):
    ...
    def predict(self, X_test_tid, X_test):
        ...
        self.eval()
        y_pred = []
        X_test_tid = torch.LongTensor(X_test_tid).to(device)
...
class resnet50():
    def __init__(self):
        ...
        self.model.fc = nn.Linear(2048, 300).to(device)
    ...
    def forward(self,xtid):
        ...
        for newid in xtid.cpu().numpy():
            ...
            img_list.append(im)
        batch_img = torch.cat(img_list, dim=0).to(device)
...
def load_dataset():
    ...
    with open(pre+ '/new_id_dic.json', 'r', encoding='utf-8') as f:
